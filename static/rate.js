...
(function () {
  ...
  const SPAN_INSTRUCTIONS = (window.SPAN_INSTRUCTIONS || "").trim();
  ...
  function refreshSpanLockFor(card, idx) {
    ...
    if (!allRated) {
      if (!hint) {
        ...
        const help = section.querySelector(".span-admin-instructions:not([hidden])");
  ...
  function initSpanInteractionsFor(card, idx) {
    ...
    card.querySelector(".toggle-span-help").addEventListener("click", () => {
      const body = card.querySelector(".span-admin-instructions");
  ...
  function buildCard(idx) {
    ...
    if (ENABLE_SPANS) {
      ...
      const adminInstr = card.querySelector(".span-admin-instructions");
      const helpToggle = card.querySelector(".toggle-span-help");
      if (adminInstr && SPAN_INSTRUCTIONS) {
        adminInstr.textContent = SPAN_INSTRUCTIONS;
        adminInstr.hidden = false;
        if (helpToggle) helpToggle.hidden = false;
      }
